...
/// init parameter keeps the property's own name. Two or more → `DataLayout` aliases
/// a tuple of them and the init takes it as one `dataLayout` parameter. Either way
/// the typealias and init are built by the same code below, driven entirely by
/// `isTuple`. Zero properties yields a plain empty init. Shared by
/// `@DataLayoutInit` and `@DataInit`.
///
...
public func renderDataLayoutMembers(properties: [StoredProperty], access: String) -> [DeclSyntax] {
    ...
    return [
        ...
        DeclSyntax(
            stringLiteral: """
                \(access)init(_ \(paramName): DataLayout) {
                \(assignments)
                }
                """
        ),
