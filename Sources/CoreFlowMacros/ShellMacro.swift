...
/// excluded because they become `Core`'s own fields (substituted or copied
/// verbatim); initializers
...
/// CLAUDE.md.) The copies compile against `Core`'s own fields by
/// construction — same names, and read-surface parity is designed in for the
/// whitelisted substitutes (`$x` is `Binding<T>` on both sides, `@Query`'s
/// fetched value reads directly on both) and trivially true for a verbatim
/// copy, because it *is* the same declaration.
///
