...
/// Swift refuses to synthesize, or any at all for a class/actor — plus two
/// tuple typealiases over the same non-private properties:
///
/// ```swift
/// @Flowable
/// public struct User {
...
///     // public typealias InFlowSplat = (UUID, Bool)          // unlabeled — splats in
...
/// - `InFlowSplat` is deliberately unlabeled so any structurally-compatible
///   tuple splats into `makeFlow(_:)` — an `InFlow`-typed value included;
///   `InFlow` is the labeled twin: the readable, `Mirror`-reflectable name
///   of the shape. One property collapses both to the bare type (Swift has
///   no 1-tuples); zero generates only `init()`.
///
/// Deliberately nothing more: no accessor reading an instance back out into
